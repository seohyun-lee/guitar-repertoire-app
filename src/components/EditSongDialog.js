...
const EditSongDialog = ({ open, onClose, song, onUpdate }) => {
  ...
  useEffect(() => {
    if (song) {  // Check if song is not null
      setFormData({
        title: song.title || '',
        artist: song.artist || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onUpdate(updatedSong);
      onClose();
    } catch (error) {
      alert('곡 정보 수정 중 오류가 발생했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px] bg-white rounded-3xl">
        <DialogHeader>
          <DialogTitle className="text-center">곡 정보 수정</DialogTitle>
        </DialogHeader>
            ...
            {formData.category === 'band' && (
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={`bg-gray-50 rounded-xl border border-gray-800 ${
                    formData.period ? 'text-gray-800' : 'text-gray-400'
                  }`}
                />
              </div>
            )}
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-sm transition-colors ${
                        formData.genres.includes(genre)
                          ? 'bg-blue-100 text-blue-700' // Selected state
